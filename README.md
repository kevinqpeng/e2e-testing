...
一个可全局安装、也可随项目共享的 Agent Skill：**主模型编排 + ego-browser 固定脚本 + 按需 Jev 判断 + 失败诊断与修复复测**。
...
1. 安装整个技能包到所需作用域，审阅并移除旧项目包装技能中指向个人主目录的引用。
2. 业务用例留在项目里，导出 `run(task, config)`；通过实际安装的 runner 的 `--scenario` 选择，项目代码不 import 全局绝对路径。
3. 将以前隐含的服务地址等改为项目配置，输出目录留在项目。以前自行写 result.json 的用例仍可接入，前提是满足当前结果契约。
4. 旧 Jev 信封缺少本次 runId 时不兼容；重新采集/判断并生成对应信封，不把旧结果伪装成本次运行。
...
测试使用明确标记的离线浏览器/模型替身，验证迁移路径、两种安装目录布局、唯一证据目录、进程失败、错误与低置信度判断、跨运行响应绑定；不冒充真实浏览器或付费模型测试。真实运行另需满足上述依赖。
